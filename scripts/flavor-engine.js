...
export class LustEngine {
  ...
  async applyStimulus({ sourceToken, observers, stimulus }) {
    ...
    for (const observer of observers) {
      ...
      if (rounded !== Lust) {
        ...
        game.dndPF2e?.flavorEngine?.tryStimulusLine?.(stimulus, sourceToken, observer);
  ...
  async _handleLustAuraEntry(sourceToken, targetToken) {
    ...
    const { traits: actorTraits, effects: effectSlugs } = game.dndPF2e?.flavorEngine?.getActiveStimuli(sourceToken.actor) ?? { traits: [], effects: [] };
    ...
    for (const stimulus of stimulusTypes) {
      ...
      if (!config || !(hasTrait || hasEffect)) {
        if (getSetting("debugStimuli")) {
        logDebug("info", `[D&Degenerates] Skipping stimulus '${stimulus}' → trait/effect not matched.`);
      }
        continue;
      }
      ...
      if (rounded !== Lust) {
        ...
        if (getSetting("debugStimuli")) {
        logDebug("info", `[D&Degenerates] 💢 Aura Lust gain applied to ${targetToken.name} from ${sourceToken.name}`);
        logDebug("info", `[D&Degenerates] ${targetToken.name} affected by ${stimulus}: +${increment.toFixed(2)} → ${rounded}`);
      }
      }
    }
  }
